...
#   scripts/test_mac.sh --bed      also build ./ionomos-testbed and print how to drive it
set -euo pipefail
cd "$(dirname "$0")/../ionomos"
PY=${PY:-python3}
...
if [ "${1:-}" = "--bed" ]; then
  echo "==> testbed"; .venv/bin/ionomos testbed init "$(pwd)/../ionomos-testbed"
