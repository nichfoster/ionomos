"""`python -m ionomos` and the PyInstaller entry point.

Wraps the CLI so a frozen exe can never die silently: any uncaught error is
written to Ionomos-crash.txt next to the exe, shown in a dialog when there
...
def _report(exc: BaseException) -> None:
    ...
    if sys.stderr is not None:
        ...
        try:
            ...
            r = tk.Tk()
            r.withdraw()
            messagebox.showerror("Ionomos crashed", text[-1500:] + (f"\n\nSaved to {where}" if where else ""))
            r.destroy()
