...
Stress test: throw a lot of messy drops and some chaos at a real watcher + worker, then check invariants.
    ...
    ionomos testbed stress [--n 60] [--seed 1] [--dir DIR] [--keep]
...
the FragPipe worker in threads with fast timings and the fake FragPipe, and:
  ...
  chaos   the worker is killed and restarted mid-search, the ledger is
          locked by another connection for 2 s, a job's ionomos.json is
...
log = logging.getLogger("ionomos.stress")
...
def run(n: int = 60, seed: int = 1, root: Path | None = None, keep: bool = False, timeout: float = 240,
    ...
    from ionomos.worker import Worker, pause, request_cancel, resume

    rng = random.Random(seed)
    rep = Report()
    tmp = None
    if root is None:
        tmp = tempfile.mkdtemp(prefix="ionomos-stress-")
        root = Path(tmp)
    root = Path(root)
    if root.exists():
        shutil.rmtree(root)
    os.environ.setdefault("IONOMOS_FAKE_FP_SECONDS", "0.3")
    os.environ.pop("IONOMOS_FAKE_FP_MODE", None)
    cfg_path = testbed.init(root / "bed")
    for s in (root / "bed" / "samples").iterdir():  # the stress drops replace the normal samples
        shutil.rmtree(s)
    cfg = load(cfg_path)
    counter = _Counter()
    logging.getLogger("ionomos").addHandler(counter)
    ...
    if chaos:
        def chaos_monkey():
            ...
            con.rollback()
            con.close()
            # 3. corrupt a queued job's ionomos.json
            for j in led.list("queued")[:1]:
                (Path(j.dest_dir) / names.STATUS_FILE).write_text("{not json", encoding="utf-8")
            # 4. pause, then resume
...
def fuzz_names(n: int = 2000, seed: int = 1) -> list[str]:
    ...
    from ionomos.naming import NamingError, find_date, group_raws, parse_raw_name, sanitize, tokens_of
