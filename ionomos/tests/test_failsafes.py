...
FragPipe error explanations, pause/cancel, graceful stop of a real `ionomos run` process."""
import json
import sqlite3
import subprocess
import sys
...
from ionomos import configio, fragpipe, health, service, testbed
...
from ionomos.worker import Worker, pause, paused, request_cancel, resume
...
def test_log_problems_extracts_warnings_and_tracebacks(tmp_path):
    lf = tmp_path / "ionomos.log"
    lf.write_text(
        "2026-09-22 10:00:00,1 INFO    ionomos: fine\n"
        "2026-09-22 10:00:01,1 WARNING ionomos.worker: job 1 waiting: x\n"
        "2026-09-22 10:00:02,1 ERROR   ionomos.watcher: scan failed\n"
        "Traceback (most recent call last):\n  File \"a.py\", line 1\nOSError: gone\n"
        "2026-09-22 10:00:03,1 INFO    ionomos: fine again\n", encoding="utf-8")
...
def test_run_process_single_instance_graceful_stop_and_recovery(bed):
    ...
    p1 = _run_proc(bed["cfg_path"], {"IONOMOS_FAKE_FP_SECONDS": "60"})
    try:
    ...
    p3 = _run_proc(bed["cfg_path"], {"IONOMOS_FAKE_FP_SECONDS": "0"})
    try:
...
def test_run_process_rebuilds_a_corrupt_ledger(bed):
    ...
    p = _run_proc(bed["cfg_path"], {"IONOMOS_FAKE_FP_SECONDS": "0"})
    try:
        ...
        assert list(bed["cfg"].database.parent.glob("ionomos.db.broken-*"))
    finally:
...
def test_diagnostics_report_and_zip(bed, monkeypatch):
    ...
    assert any(n.endswith("ionomos_run/fragpipe_console.log") for n in names)
    assert not any(n.endswith(".raw") for n in names)
    rec = json.loads(zipfile.ZipFile(z).read(next(n for n in names if n.endswith("ionomos.json"))))
...
def test_intake_never_loops_on_a_bad_folder(bed, monkeypatch, case):
    ...
    from ionomos import intake as intake_mod
