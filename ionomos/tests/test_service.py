...
def test_config_path_memory(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("IONOMOS_CONFIG", "/env/c.yaml")
...
def test_run_cli_and_start_stop(tmp_path):
    ...
    assert code == 0 and "Ionomos" in out
    cmd = service.ionomos_command()
    assert cmd[0] == sys.executable and cmd[1:] == ["-m", "ionomos.cli"]
...
def clone(tmp_path):
    ...
    (seed / "ionomos").mkdir()
    (seed / "ionomos" / "pyproject.toml").write_text("[project]\nname='x'\n", encoding="utf-8")
...
def test_source_checkout_finds_repo_root():
    ...
    assert root is not None and (root / "ionomos" / "pyproject.toml").is_file()


def test_diagnostics_contains_everything(lab):
    cfg = lab["cfg_path"]
    log_dir = lab["auto"] / "logs"
    (log_dir / "ionomos.log").write_text("line1\nline2\n", encoding="utf-8")
    (lab["inbox"] / "X.REJECTED.txt").write_text("because\n", encoding="utf-8")
    text, where = service.save_diagnostics(cfg)
    for needle in ("ionomos ", "=== check", "=== status --all", "=== config.yaml", "line2", "=== inbox",
