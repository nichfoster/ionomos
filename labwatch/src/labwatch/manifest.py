...
from labwatch.naming import NamingError, RawName, RawSet
...
def save_overrides(folder: Path, ov: Overrides) -> Path:
    ...
    header = "# Written by labwatch. Edit freely; keys are documented in docs/NAMING_CONVENTION.md\n"
...
def apply_file_overrides(raws: RawSet, ov: Overrides) -> RawSet:
    ...
    if not ov.files:
        return raws
    from labwatch.naming import group_from_parsed  # local import avoids cycle at module load
