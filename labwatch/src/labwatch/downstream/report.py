...
from labwatch.downstream import charts
from labwatch.downstream.analysis import DiffResult
from labwatch.downstream.quant import QuantMatrix
...
def methods_text(m: QuantMatrix, diffs: list[DiffResult], method: str, fragpipe_note: str) -> str:
    ...
    parts = [f"Raw files were searched with FragPipe{(' (' + fragpipe_note + ')') if fragpipe_note else ''} using the "
             f"lab's pinned {escape(method)} workflow, run automatically by LabWatch."]
    if m.kind == "ratio":
...
def render(ctx: dict, m: QuantMatrix | None, diffs: list[DiffResult], notes: list[str], files: list[str]) -> str:
    ...
    meta = " · ".join(x for x in (ctx.get("user"), ctx.get("method"), ctx.get("date"),
                                  f"generated {datetime.now():%Y-%m-%d %H:%M}", f"LabWatch {ctx.get('version', '')}") if x)
    ...
    return (f"<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' "
            f"content='width=device-width,initial-scale=1'><title>{escape(title)} — LabWatch report</title>"
