"""
After a successful FragPipe run: the downstream analysis (labwatch.downstream).
...
becomes a warning on the done job, and `labwatch analyze <folder>` re-runs it.
"""
...
def run_for_folder(dest: Path, cfg, method: str | None = None, extra: dict | None = None):
    """Analyse one experiment folder (used after a job and by `labwatch analyze`). Returns downstream.Outcome."""
    from labwatch import downstream

    dest = Path(dest)
    try:
        record = json.loads((dest / "labwatch.json").read_text(encoding="utf-8"))
    ...
    try:  # an experiment.yaml edited after intake (e.g. new comparisons) wins
        from labwatch.manifest import load_overrides
