...
status and what to do about it. Shared by the app's Setup tab and `labwatch init`.
...
def writable(folder: Path) -> tuple[bool, str]:
    """Create and delete a probe file (labwatch's own, never user data)."""
    if not folder.is_dir():
        return False, "does not exist"
    probe = folder / f".labwatch-write-test-{uuid.uuid4().hex[:8]}"
    try:
...
def run(config_path: Path, data: dict | None = None, probe_watcher: bool = True) -> list[Item]:
    from labwatch import configio, fragpipe, health, service
    from labwatch.config import ConfigError, layout_problems, load
    ...
    if cfg is not None:
        ...
        if os.name == "nt" or sys.platform == "win32":
            ...
            add(Item("task", "Starts automatically at logon", "ok" if st == "installed" else "todo",
                     "installed" if st == "installed" else "not installed",
                     "" if st == "installed" else "Install startup task.", None if st == "installed" else "install_task"))
    return items
...
def summary(items: list[Item]) -> str:
    ...
    if not blocking:
        return f"All {len(items)} checks pass — LabWatch is ready."
