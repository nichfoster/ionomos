...
def build_sample(dest_parent: Path, name: str, size: int = 4096) -> Path:
    ...
    if spec.get("yaml"):
        (d / "experiment.yaml").write_text(yaml.safe_dump(spec["yaml"]))
    return d
...
lines = Path(a.manifest).read_text().splitlines() if a.manifest and Path(a.manifest).is_file() else []
print(f"FAKE FragPipe: {len(lines)} raw files")
for i in range(3):
    print("working...", i + 1, "/ 3"); sys.stdout.flush(); time.sleep(1)
(wd / "fragpipe.workflow").write_text(Path(a.workflow).read_text() if a.workflow and Path(a.workflow).is_file() else "")
...
def init(root: Path, slow_defaults: bool = False) -> Path:
    ...
    fake_py.write_text(FAKE_FRAGPIPE_PY)
    if os.name == "nt":
    ...
    (root / "README.txt").write_text(_readme(root, cfg_path))
    return cfg_path
...
def main(args) -> int:
    if args.tb_cmd == "init":
        ...
        print((Path(args.dir).resolve() / "README.txt").read_text())
        print(f"config: {cfg}")
        return 0
    if args.tb_cmd == "list":
