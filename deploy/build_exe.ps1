<#
Build LabWatch.exe + labwatch-cli.exe on Windows with PyInstaller, then a release zip.
...
Copy-Item dist\exe\LabWatch.exe, dist\exe\labwatch-cli.exe, docs\DEPLOY_WINDOWS.md, docs\NAMING_CONVENTION.md $out
...
labwatch-cli.exe is the same program for the command line:  labwatch-cli.exe check | status | dry-run <folder>
If LabWatch.exe ever fails to open, look for LabWatch-crash.txt next to it.
