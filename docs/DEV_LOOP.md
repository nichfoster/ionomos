# The Mac ↔ PC loop (while LabWatch is being prototyped)
...
```
 ...
 ┌──────────────────┐  push   ┌──────────────┐   pull    ┌───────────────────────────┐
 │ edit + tests     │ ──────▶ │ private repo │ ────────▶ │ C:\labwatch-src  (clone)  │
 │ (Claude)         │         │              │           │   └─ .venv  (editable)    │
 └──────────────────┘         └──────────────┘           │ LabWatch app ← runs from  │
        ▲                                                │   the clone directly      │
...
files in `C:\labwatch-src` instead of a copy. So `git pull` *is* the update.
...
2. Right-click PowerShell → Run, then:
   ...
   ```
   It installs git if missing (asks first), clones the repo to `C:\labwatch-src`
   (a browser window asks you to sign in to GitHub once — it's a private
   repo), makes the Python environment, puts a **LabWatch** shortcut on the
...
|---|---|
| Describe the problem to Claude (paste the diagnostics), it fixes + tests, then `git push` | Open LabWatch → tab 5 → the *Development install* box says **UPDATE AVAILABLE** → **Update from GitHub & restart** |
| | Test again |
...
From a terminal the same thing is `C:\labwatch-src\UPDATE.ps1`.
...
- **Never edit code on the PC.** Update refuses to run if files in the clone
  ...
  Mac, push, update.
- **Everything of yours lives outside `C:\labwatch-src`.** If the clone is
...
→ Actions builds `LabWatch-0.2.0-windows.zip` on a Windows machine and attaches
